...
import { TransportPrice } from '../types';
import { Button } from '@/components/ui/button';
import { Card } from '@/components/ui/card';
import { Input } from '@/components/ui/input';

interface TransportPricesTableProps {
  prices: TransportPrice[];
  searchTerm: string;
  setSearchTerm: (term: string) => void;
  onAdd: () => void;
  onEdit: (price: TransportPrice) => void;
  onDelete: (id: number) => void;
  isLoading: boolean;
}

const TransportPricesTable: React.FC<TransportPricesTableProps> = ({
  prices,
  searchTerm,
  setSearchTerm,
  onAdd,
      ...
      <div className="p-4 border-b flex items-center justify-between flex-col sm:flex-row gap-4 sm:gap-0">
        <div className="flex items-center space-x-2 w-full sm:w-auto">
          ...
          >
            ...
            <span>Add Transportation Price</span>
          </Button>
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto">
          <div className="relative flex-1 sm:flex-none">
            ...
            <Input
              ...
              placeholder="Search city, state, port..."
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b">
              ...
              <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">City 2</th>
              <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">Zip</th>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={8} className="text-center py-4">No transportation prices found</td>
              </tr>
            ) : (
              prices.map((price) => (
                <tr key={price.id} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-600">{price.id}</td>
                  <td className="px-4 py-3 text-gray-600">{price.city}</td>
                  <td className="px-4 py-3 text-gray-600">{price.city2}</td>
                  <td className="px-4 py-3 text-gray-600">{price.zip}</td>
...
export default TransportPricesTable;
